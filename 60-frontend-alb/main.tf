resource "aws_lb" "public_alb" {
  name               = "${local.common_name}-public-alb" # roboshop-dev-frontend-alb
  internal           = false
  load_balancer_type = "application"
  security_groups    = [local.public_alb_sg_id]
  subnets            = local.public_subnet_ids

  enable_deletion_protection = false # usually true, but while practicing make it false
...
resource "aws_route53_record" "www" {
  zone_id = var.zone_id
  name    = "${var.project}-${var.environment}.devopswithrani.online" # roboshop-dev.devopswithrani.online
  type    = "A"
